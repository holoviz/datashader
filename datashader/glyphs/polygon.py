...
class PolygonAwkwardGeom(_GeometryLike):
    @property
    def geom_dtypes(self):
        return (AwkwardDtype,)
    ...
    def _build_extend(self, x_mapper, y_mapper, info, append):
        ...
        perform_extend_cpu = _build_extend_awkward_polygon_geometry(
            draw_polygon, expand_aggs_and_cols
        )
        geom_name = self.geometry
...
def _build_draw_awkward_polygon(append, map_onto_pixel, x_mapper, y_mapper, expand_aggs_and_cols):
    @ngjit
    @expand_aggs_and_cols
    def draw_polygon(
            i, sx, tx, sy, ty, xmin, xmax, ymin, ymax,
            xs, ys, yincreasing, eligible,
            polygon,
            *aggs_and_cols
    ):
        """Draw a polygon using a winding-number scan-line algorithm
        """
        nppoly = np.asarray(polygon)
        xvalues = nppoly[::2]
        yvalues = nppoly[1::2]

        poly_xmin = np.min(xvalues)
        poly_ymin = np.min(yvalues)
        poly_xmax = np.max(xvalues)
        poly_ymax = np.max(yvalues)
        ...
        ei = 0
        for k in range(0, len(nppoly) - 2, 2):
            x0 = nppoly[k]
            y0 = nppoly[k + 1]
            x1 = nppoly[k + 2]
            y1 = nppoly[k + 3]
...
def _build_extend_awkward_polygon_geometry(
        draw_polygon, expand_aggs_and_cols
):
    def extend_cpu(
            sx, tx, sy, ty, xmin, xmax, ymin, ymax,
            geometry,  # AwkwardExtensionArray
            *aggs_and_cols
    ):
        ...
        for i in eligible_inds:
            ...
            for multipoly in item:
                for polygon in multipoly:
                    draw_polygon(i, sx, tx, sy, ty, xmin, xmax, ymin, ymax,
                                 xs, ys, yincreasing, eligible, polygon,
                                 *aggs_and_cols)
